...
# tl conversion to kbps (formula provided in documentation)
...
for(i in 1:nrow(conv)){
  ...
  sums_list[[i]] <- sub1 %>% 
    group_by(RIDRETH1) %>% 
    summarize(mean_val = mean(plasma_var, na.rm = TRUE), sd_val = sd(plasma_var, na.rm = TRUE))
...
# lab measure summaries
names(sums_list) <- conv[, 2]
plas_s1 <- do.call(rbind.data.frame, sums_list)
plas_s1$var <- row.names(plas_s1)
plas_s1$display <- paste0(round(plas_s1$mean_val, 2), " (", round(plas_s1$sd_val, 2), ")")
print(plas_s1 %>% select(RIDRETH1, var, display), n = 40)
...
ggsave(p, filename = "Comparison_Project/Manuscript/ClinEpi/Plots/NHANES_Evaluation_Fig1.png", 
...
p <- (plot1 | plot2) + plot_layout(nrow = 2) + plot_annotation(tag_levels = 'A')
ggsave(p, filename = "Comparison_Project/Manuscript/ClinEpi/Plots/NHANES_Evaluation_FigS1.png", 
       width = 10, height = 8, units = "in", dpi = 500)
...
ggsave(p, filename = "Comparison_Project/Manuscript/ClinEpi/Plots/NHANES_Evaluation_Fig2.png", 
...
ggsave(p, filename = "Comparison_Project/Manuscript/ClinEpi/Plots/NHANES_Evaluation_Fig3.png", 
       width = 12, height = 8, units = "in", dpi = 500)
```
...
ggsave(p, filename = "Comparison_Project/Manuscript/ClinEpi/Plots/NHANES_Evaluation_FigS3.png", 
       width = 12, height = 8, units = "in", dpi = 500)
...
ggsave(p, filename = "Comparison_Project/Manuscript/ClinEpi/Plots/NHANES_Evaluation_FigS4.png", 
       width = 12, height = 8, units = "in", dpi = 500)
```
...
for(i in 1:nrow(conv)){
  ...
  ggsave(p, filename = paste0("Comparison_Project/Manuscript/ClinEpi/Plots/FigS2/Correlations_", conv[i, 3],".png"), 
