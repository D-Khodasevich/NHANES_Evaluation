...
a <- ggplot(subset(tab, Name == "Horvath")) + 
  geom_pointrange(aes(y = fct_inorder(sig_display), x = Estimate, xmin = CI_low, 
                      xmax = CI_up, color = term), size = 0.8, linewidth = 1.5) + 
...
full <- a+b+c+d+e+f+g+h+i+j+k+l+m+n+o +
  ...
  legend.key.size = unit(2, 'cm'),legend.key.height = unit(1, 'cm'), 
  legend.text = element_text(size=14)) & guides(color=guide_legend(nrow=2))
...
full <- a+b+c+d+e+f+g+h+i+j+k+l+m+n+o +
  plot_layout(ncol = 3, guides = 'collect') & theme(legend.position = "bottom", 
  legend.key.size = unit(2, 'cm'),legend.key.height = unit(1, 'cm'), 
  legend.text = element_text(size=14)) & guides(color=guide_legend(nrow=1))
...
telo_sub$term <- factor(telo_sub$term, levels = c("Phenotype", "Phenotype*Mexican",
"Phenotype*NH Black", "Phenotype*Other", "Phenotype*Other Hispanic"))
h <- ggplot(telo_sub) + 
...
full <- a+b+c+d+e+f+g+(h + guides(color = "none"))+l+m+n+o +
  plot_layout(ncol = 3, guides = 'collect') & theme(legend.position = "bottom", 
  legend.key.size = unit(2, 'cm'),legend.key.height = unit(1, 'cm'), 
  legend.text = element_text(size=14)) & guides(color=guide_legend(nrow=2))
