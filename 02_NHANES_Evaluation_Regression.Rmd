...
table_df %>% filter(facet == "RaceEthnicity") %>% select(facet, Name, term, display, p_display, p_ind)
table_df %>% filter(facet != "RaceEthnicity") %>% select(facet, Name, term, display, p_display, p_ind)
...
ggsave(full, filename = "Comparison_Project/Manuscript/ClinEpi/Plots/Fig4.png", 
...
full <- a+b+c+d+e+f+g+h+i+j+k+l+m+n+o +
  plot_layout(ncol = 3, guides = 'collect') & theme(legend.position = "bottom")
ggsave(full, filename = "Comparison_Project/Manuscript/ClinEpi/Plots/FigS5.png", 
       width = 12, height = 9, units = "in", dpi = 500)
```
